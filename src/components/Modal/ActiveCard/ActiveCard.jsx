...
import { clearCurrentActiveCard } from '~/redux/activeCard/activeCardSlice'
...
function ActiveCard() {
  ...
  const callUpdateCardAPI = async (updateData) => {
    const updatedCard = await updateCardDetailsAPI(activeCard._id, updateData)
    dispatch(updateCurrentActiveCard(updatedCard))
    // TODO: update card in active board
  ...
  const onUploadCardCover = (event) => {
    ...
    // Gọi API...
    toast.promise(
      callUpdateCardAPI(reqData).finally(() => event.target.value = ''),
      { pending: 'Updating...' }
    )
          ...
          <Grid xs={12} sm={9}>
            ...
            <Box sx={{ mb: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                <SubjectRoundedIcon />
                <Typography variant="span" sx={{ fontWeight: '600', fontSize: '20px' }}>{activeCard?.description}</Typography>
              </Box>
